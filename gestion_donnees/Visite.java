...
public class Visite {
	...
	public Visite(String idVisite, String intitule, String dateVisite, String heureVisite, Exposition expositionLiee, Employe employeLiee, Conferencier conferencier, String numTel) throws VisiteException, EmployeException {
		...
		if (employeLiee.getId().length() != 7 || employeLiee.getId() == null) {
			throw new VisiteException();
		} else {
			Employe employeTrouve = null;
			for (Employe employe : gestionDonnees.employes) {
				if (employe.getId() == employeLiee.getId()) {
					employeTrouve = employe;
				}
			}
			if (employeTrouve != null) {
			     this.employe = employeLiee;
			 } else {
			     throw new VisiteException();
			 }
		}
		
		if (expositionLiee.getId().length() != 7 || expositionLiee.getId() == null) {
			throw new VisiteException();
		} else {
			Exposition expositionTrouvee = null;
			
			for (Exposition exposition : gestionDonnees.expositions) {
			        if (exposition.getId() == expositionLiee.getId()) {
			            expositionTrouvee = exposition;
			        }
			}
			 if (expositionTrouvee != null) {
			     this.exposition = expositionLiee;
			 } else {
			     throw new VisiteException();
			 }
		}
	...
	public static void main(String args[]) throws VisiteException, EmployeException, ExpositionException, ConferencierException {
		...
		ArrayList<Date> indisponibilite = new ArrayList<>();
	    	...
	    	Employe employeLiee = new Employe("1111111", "Clement", "juery", "1234");
	    
	    	Exposition expositionLiee = new Exposition("E000002", "salut", "dzayudfgauyfaf", 6, true, tab, "1984", "1987", "12/10/2012", "12/12/2024");
